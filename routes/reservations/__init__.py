...
_RESERVE_MENU_CACHE = {"loaded_at": 0.0, "items": None, "categories": None}

def _get_menu_items_and_categories_for_reserve_menu():
    # Small TTL cache to make /reserve/menu feel snappy on back/forward & tab switches
    ttl_seconds = 10
    now = time.monotonic()
    cached_items = _RESERVE_MENU_CACHE["items"]
    cached_categories = _RESERVE_MENU_CACHE["categories"]
    if cached_items is not None and cached_categories is not None and (now - _RESERVE_MENU_CACHE["loaded_at"]) < ttl_seconds:
        return cached_items, cached_categories
    ...
    _RESERVE_MENU_CACHE["items"] = menu_items
    _RESERVE_MENU_CACHE["categories"] = categories
    _RESERVE_MENU_CACHE["loaded_at"] = now
...
def reserve_menu():
    ...
    # Determine minimum price for Exclusive (Consumable logic)
    is_exclusive = pending.get('booking_type') == 'EXCLUSIVE'
    min_price = Decimal('0')
    if is_exclusive:
        try:
            res_date = datetime.strptime(pending['date'], '%Y-%m-%d').date()
            is_weekend = res_date.weekday() >= 5
            min_price = Decimal('30000') if is_weekend else Decimal('20000')
        except:
            pass
...
def reserve_payment():
    ...
    # Calculate Grand Total with Minimum for Exclusive
...
def reserve_confirm():
    ...
    # Build order items (batch fetch to avoid N+1 queries)
    order_items_list = []
    food_total = Decimal('0')
    item_ids = []
    for item_id_str, qty in selected_items.items():
    ...
    menu_items = (
        MenuItem.query.options(load_only(MenuItem.id, MenuItem.price, MenuItem.is_available))
        .filter(MenuItem.id.in_(item_ids))
        .all()
    )
    menu_items_by_id = {m.id: m for m in menu_items}
    ...
    for item_id_str, qty in selected_items.items():
        ...
        menu_item = menu_items_by_id.get(item_id)
        if menu_item and menu_item.is_available:
            subtotal = Decimal(str(menu_item.price)) * int(qty)
            food_total += subtotal
    ...
    # Apply Exclusive Booking Minimum Pricing
    if booking_type == 'EXCLUSIVE':
        is_weekend = res_date.weekday() >= 5  # 5=Saturday, 6=Sunday
        min_price = Decimal('30000') if is_weekend else Decimal('20000')
        
        if food_total < min_price:
            # We charge the minimum price even if menu selection is less
            food_total = min_price
